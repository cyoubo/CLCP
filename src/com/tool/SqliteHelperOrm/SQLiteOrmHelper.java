...
public class SQLiteOrmHelper extends OrmLiteSqliteOpenHelper
{
	...
	public static void ToastShowResult(Context centext,int result)
	{
		if(result!=-1)
			Toast.makeText(centext, "存储成功", Toast.LENGTH_SHORT).show();
		else
			Toast.makeText(centext, "存储失败", Toast.LENGTH_SHORT).show();
	}
	
	public static void ToastShowResult(Context centext,boolean result)
	{
		if(result)
			Toast.makeText(centext, "存储成功", Toast.LENGTH_SHORT).show();
		else
